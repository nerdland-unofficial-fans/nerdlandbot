...
class Calendar(commands.Cog, name="Event Calendar"):
    ...
    async def add_event(self, ctx: commands.Context, input_text: str) -> None:
        """
        ...
        if len(input_text_split) != 3 or len(input_text_split) != 4:
        ...
        try:
            start_date = datetime.datetime.strptime(input_text[2], "%d/%m/%Y %H:%M")
        except ValueError:
            await ctx.send(translate("add_event_invalid_date", await culture(ctx)))
            return

        if len(input_text_split) == 4:
